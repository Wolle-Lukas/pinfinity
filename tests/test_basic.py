...
class TestBasicSave:
    def test_create_new_training(self, client, restore_basic_list):
        r = client.post("/api/basic/save", json=SAMPLE_TRAINING)
    ...
    def test_create_assigns_new_id(self, client, restore_basic_list):
        r1 = client.post("/api/basic/save", json=SAMPLE_TRAINING)
        r2 = client.post("/api/basic/save", json=SAMPLE_TRAINING)
    ...
    def test_update_existing_training(self, client, restore_basic_list):
        ...
        r = client.post("/api/basic/save", json=updated)
        assert r.status_code == 200
...
class TestBasicSetFavourite:
    def test_set_favourite(self, client, restore_basic_list):
        r = client.post("/api/basic/setFavourite", json={"id": 1, "favourite": 1})
        assert r.status_code == 200
        assert r.json()["code"] == 200
        assert r.json()["msg"] == "SUCCESS"

    def test_unset_favourite(self, client, restore_basic_list):
        client.post("/api/basic/setFavourite", json={"id": 1, "favourite": 1})
        r = client.post("/api/basic/setFavourite", json={"id": 1, "favourite": 0})
        assert r.status_code == 200
    ...
    def test_missing_favourite_field_returns_400(self, client):
        r = client.post("/api/basic/setFavourite", json={"id": 1})
        assert r.status_code == 400

    def test_missing_id_field_returns_400(self, client):
        r = client.post("/api/basic/setFavourite", json={"favourite": 1})
        assert r.status_code == 400

    def test_invalid_json_returns_400(self, client):
        r = client.post(
            "/api/basic/setFavourite",
            content="not-json",
            headers={"Content-Type": "application/json"},
        )
        assert r.status_code == 400
...
class TestBasicDelete:
    def test_delete_training(self, client, restore_basic_list):
        ...
        r = client.request("DELETE", "/api/basic/delete", json={"id": new_id})
        assert r.status_code == 200
        assert r.json()["code"] == 200
        assert r.json()["msg"] == "SUCCESS"
    ...
    def test_deleted_training_not_in_list(self, client, restore_basic_list):
        ...
        client.request("DELETE", "/api/basic/delete", json={"id": new_id})
    ...
    def test_missing_id_returns_400(self, client):
        r = client.request("DELETE", "/api/basic/delete", json={})
        assert r.status_code == 400
